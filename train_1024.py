...
def main():
    d_model = wgan_gp_1024.get_discriminator_model(IMG_SHAPE)
    d_model.summary()
    g_model = wgan_gp_1024.get_generator_model(NOISE_DIM)
    g_model.summary()
    
    generator_optimizer = keras.optimizers.Adam(learning_rate=0.0002, beta_1=0.5, beta_2=0.9)
    discriminator_optimizer = keras.optimizers.Adam(learning_rate=0.0001, beta_1=0.5, beta_2=0.9)

    wgan = wgan_gp_1024.WGAN(
...
class GANMonitor(keras.callbacks.Callback):
    def __init__(self, num_img=3, latent_dim=128, log_dir="", save_freq=20):
